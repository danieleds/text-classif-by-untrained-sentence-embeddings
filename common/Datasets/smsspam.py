...
class SMSSpamDataset(Dataset):
    ...
    url = 'https://github.com/danieleds/text-classif-by-untrained-sentence-embeddings/releases/download/0.1/SMSSpamCollection-3.pth'

    def __init__(self, examples: List[Tuple[torch.Tensor, torch.ByteTensor]]):
        super(SMSSpamDataset, self).__init__()
    ...
    def load(cls, root='.data'):
        ds_path = os.path.join(root, 'SMSSpam', 'SMSSpamCollection-3.pth')
        if not os.path.isfile(ds_path):
            if not os.path.exists(os.path.dirname(ds_path)):
                os.makedirs(os.path.dirname(ds_path))
            print('downloading dataset')
            torchtext.utils.download_from_url(SMSSpamDataset.url, ds_path)
    ...
    @classmethod
    def merge_folds(cls, folds: List['SMSSpamDataset']):
