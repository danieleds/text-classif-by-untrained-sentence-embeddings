...
class GridCellMeasurements:
    ...
    extra: dict = None  # For extra measures. Only measurements (e.g accuracy, f1, cross-entropy), no other information.
...
def matthews_corrcoef(predicted: torch.Tensor, expected: torch.Tensor) -> float:
    """
    ...
    :return:
    """
    return sklearn.metrics.matthews_corrcoef(expected.cpu().numpy(), predicted.cpu().numpy())
