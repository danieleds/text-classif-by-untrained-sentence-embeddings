...
class ESNModelSMS(torch.nn.Module):
    ...
    def __init__(self, input_size, reservoir_size, contractivity_coeff=0.9,
                 ...
                 hp=None):
        super(ESNModelSMS, self).__init__()
    ...
    def performance_f1(self, train_fold, val_fold, test_fold=None):
        with torch.no_grad():
            ...
            # Compute performance measures
            train_f1 = common.macro_f1_score(train_out, train_expected)
            val_f1 = common.macro_f1_score(val_out, val_expected) if val_fold else 0
            test_f1 = common.macro_f1_score(test_out, test_expected) if test_fold else 0

            return train_f1, val_f1, test_f1

    def performance_mcc(self, train_fold, val_fold, test_fold=None):
        with torch.no_grad():
            ...
            # Compute performance measures
            train_mcc = common.matthews_corrcoef(train_out, train_expected)
            val_mcc = common.matthews_corrcoef(val_out, val_expected) if val_fold else 0
            test_mcc = common.matthews_corrcoef(test_out, test_expected) if test_fold else 0
    ...
    @staticmethod
    def ensemble_performance_f1(predictions, expected):
        out = torch.stack(predictions).mean(dim=0)
        out = out.argmax(dim=1)
        return common.macro_f1_score(out, expected)


#class ESNModelSMSEnsemble(torch.nn.Module):
#
#    def __init__(self, n_models, input_size, reservoir_size, f=torch.tanh, alpha=1e-6,
#                 rescaling_method='norm', hp=None):
#        super(ESNModelSMSEnsemble, self).__init__()
#
#        self.models = [ ESNModelSMS(
#            input_size,
#            reservoir_size,
#            f=f,
#            alpha=alpha,
